...
// @downloadURL     https://raw.githubusercontent.com/aoheart/Userscripts/refs/heads/dist/links-from-zula.user.js
// @updateURL       https://raw.githubusercontent.com/aoheart/Userscripts/refs/heads/dist/links-from-zula.user.js
// @match           https://zula.link-map.jp/*
...
{ id: "amazondl", label: "Amazonデジタルミュージックストア", isDefaultEnabled: true, namePatterns: ["Amazonデジタルミュージック"], linkTypes: [77] },
  ...
  function findServiceByLinkName(linkName) {
  ...
  async function collectUrls(onProgress) {
    const anchors = Array.from(document.querySelectorAll("a.streaming-store-link-btn[data-link-url][data-link-name], a.download-store-link-btn[data-link-url][data-link-name]"));
  ...
  function openSettingsModal(clearCache) {
    const anchors = Array.from(document.querySelectorAll("a.streaming-store-link-btn[data-link-name], a.download-store-link-btn[data-link-name]"));
    const linkNamesOnPage = anchors.filter((a) => {
      try {
        return !EXCLUDED_HOSTS.has(new URL(a.dataset.linkUrl ?? "").hostname);
      } catch {
        return true;
      }
    }).map((a) => a.dataset.linkName ?? "").filter(Boolean);
    const unknownLabelsOnPage = linkNamesOnPage.filter((n) => !findServiceByLinkName(n));
    const enabledIds = settings.getEnabledIds();
    document.body.appendChild(
      buildSettingsModal({
        services: SERVICE_DEFINITIONS.map((s) => ({ id: s.id, label: s.label, isHarmony: s.isHarmony })),
  ...
  function injectUI() {
    const mountTarget = document.querySelector(".store-link-content");
